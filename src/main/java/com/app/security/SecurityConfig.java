...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http.csrf().disable().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and(). // Cors
				...
				.antMatchers("/login", "/register", "/files/download/**", "/contactmessage/visitors",
						"/files/display/**", "/car/visitors/**", "/actuator/info", "/actuator/health")
	...
	public WebMvcConfigurer corsConfigurer() {
		return new WebMvcConfigurer() {
			...
			public void addCorsMappings(CorsRegistry registry) {
				registry.addMapping("/**").allowedOrigins("*"). // "http:127.0.0.1/8080 diye spesific adresden gelenleri
																// kabul et de diyebiliriz
